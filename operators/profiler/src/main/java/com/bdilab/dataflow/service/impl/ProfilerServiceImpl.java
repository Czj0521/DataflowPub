...
public class ProfilerServiceImpl implements ProfilerService {
  ...
  /**
   * Profiler infomation of n columns.
   *
  ...
  /**
   * get columnType of n columns.
   *
   * @return for a column: {"columnName": ,"columnType":}
   */
  public List<Map<String, Object>> getColumnType(String datasource,
                                                 List<String> profilerColumnList) {
  ...
  /**
   * get columnStatistics  info of n columns.

   *
   * @return for a column: {"min": ,"minVal":,......}
   */
  public List<Map<String, Object>> getColumnInfo(String datasource,
    ...
    for (Map<String, Object> columnnInfo : columnInfoList) {
      ...
      if (dataType.equals("numeric")) {
        sql.append("avg(" + columnName + "),max(" + columnName + "),min(" + columnName + "),"
            + "stddevPop(" + columnName + "),");
        ...
        columnInfoList.get(i).put("inf",
            ((BigInteger) mapNull.get("a2.count()")).longValue() / countRow);
        columnInfoList.get(i).put("-inf",
            ((BigInteger) mapNull.get("a3.count()")).longValue() / countRow);
        columnInfoList.get(i).put("zeros",
            ((BigInteger) mapNull.get("a4.count()")).longValue() / countRow);
        ...
        sql.append(columnName + ",count(*) from " + datasource + " group by "
            + columnName + " order by count()" + " desc limit 3");
  ...
  /**
   * get pixochart information of n columns.

   *
   * @return for a column: {"calibration": ,"countCabration": }
   */
  public List<Map<String, Object>> getPixo(String datasource,
                                           List<Map<String, Object>> columnInfoList) {
    for (Map<String, Object> columnInfo : columnInfoList) {
      Map<String, Object> sqlAndCalibration =
          pivotChartJobUtils.getSqlAndCalibration(datasource, columnInfo);
      ...
      if (DataTypeEnum.CLICKHOUSE_DATATYPE_MAP.get(columnType).equals("string")) {
        ...
        for (String sql : (ArrayList<String>) sqlAndCalibration.get("sql")) {
          List<Map<String, Object>> maps = clickHouseJdbcUtils.queryForList(
              "select count(*) from " + datasource + " where " + sql);

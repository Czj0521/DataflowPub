...
public class PivotChartSqlGeneratorTest {
  ...
  public void testGenerateSql_Success() {
    ...
    assert sql.equals("select * from ("
            ...
            + " select roundDown(DealerId,[0.0, 50.0, 100.0, 150.0, 200.0, 250.0, 300.0, 350.0, "
            + "400.0, 450.0, 500.0, 550.0, 600.0, 650.0, 700.0, 750.0, 800.0, 850.0]) DealerId_bin"
            + ",roundDown(CarMileage,[0.0, 4580.0, 8013.0, 11433.0, 14916.0, 18506.0, 22262.0, "
            + "26242.0, 30499.0, 35089.0, 40092.0, 45684.0, 52166.0, 60073.0, 70859.0, 106806.0])"
            + " CarMileage_bin,date_trunc('month', SalesDate) AS SalesDate_bin,"
  ...
  public void testGenerateSql_Percentage() {
    ...
    assert sql.equals("WITH (select sum(Model_count) from"
            ...
            + " select Type,ExteriorColor,CarYear,count(Model) Model_count,"
            + "round(Model_count/S*100,2) \"%\""
            + " from dataflow.car"
            + " group by Type,ExteriorColor,CarYear order by Type,ExteriorColor,CarYear");
  }

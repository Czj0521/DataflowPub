    ...
    useEffect(() => {
        const g = new Graph({
            ...
            onPortRendered(args) {
                ...
                if (container) {
                  if(args.port.group === 'in'){
                    ReactDOM.render(
                      <MyPorts node={args.node} port={args.port} graph={graph}/>,
                      container,
                    )
                  }else{
                    // ReactDOM.render(
            ...
            connecting: {
              ...
              allowPanning:(e)=>{
                // console.log(e)
                if(e.target.classList.length === 0 || e.target.classList[0] === 'react-grid-layout' || e.target.className === 'hetu_sidebar_item'){
                  return false
                }
                return true
              }
    ...
    useEffect(()=>{
      if(graph){
        ...
        graph.on('node:mousedown',({e,node})=>{
          // console.log(e)
          if(e.target instanceof SVGElement || e.target.offsetParent.className === 'myports_wrapper' || e.target.className === 'hetu_sidebar_item'){
            node.setData({
              move:false
            })
          }else{
        ...
        graph.on('edge:removed',({cell,edge,options})=>{
          ...
          if(target && source){
            ...
            let filter = JSON.parse(JSON.stringify(targetData.filter))
    ...
    const childrenFilter = (current,children) => {
      ...
      if(JSON.stringify(current.filter) === '{}'){
        if(fieldType[current.type] === 'number'){
          ...
          const obj = {}
          const arr = []
          data.map(i=>i[current.type]).forEach((item,i)=>{
      ...
      <div>
        <Row className='hetu_analysis'>
            ...
            <Col className='hetu_content' span={19}>
                ...
                <div className='hetu_minimap' id='miniMap'></div>
            </Col>
        </Row>
        <div></div>
      </div>
